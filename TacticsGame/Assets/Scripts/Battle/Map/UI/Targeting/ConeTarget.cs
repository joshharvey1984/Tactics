...
namespace TacticsGame.Battle.Map.UI.Targeting {
    public class ConeTarget
    {
        ...
        private void DrawCone(int length, int degrees) {
            ...
            //_coneTarget.AddComponent<MeshRenderer>();
        ...
        private List<MapTile> FindTilesInCone()
        {
            ...
            var vPos0 = new Vector3(_coneTarget.transform.TransformPoint(vert[0]).x, _coneTarget.transform.TransformPoint(vert[0]).z);
            var vPos1 = new Vector3(_coneTarget.transform.TransformPoint(vert[1]).x, _coneTarget.transform.TransformPoint(vert[1]).z);
            var vPos2 = new Vector3(_coneTarget.transform.TransformPoint(vert[2]).x, _coneTarget.transform.TransformPoint(vert[2]).z);
        ...
        private static float Sign (Vector3 p1, Vector3 p2, Vector3 p3) => (p1.x - p3.x) * (p2.y - p3.y) - (p2.x - p3.x) * (p1.y - p3.y);
